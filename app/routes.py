...
def product_area(id):
    if request.method == 'GET':
        product_area = ProductArea()
        product_area = product_area.find_one(id)
        ...
        data = request.get_json()
        product_area = ProductArea()
        product_area.id = data['id']
        product_area.name = data['name']
        product_area = product_area.update()
        return jsonify(product_area.to_dict())
    else:
        data = request.get_json()
        product_area = ProductArea()
...
@app.route('/feature-requests/', methods=['GET', 'PUT', 'DELETE'])
def feature_request():
            ...
            if feature_id:
                data = FeatureRequest.to_collection_dict(
                    FeatureRequest.query.filter_by(
                        feature_id=feature_id), page, per_page, 'feature_request', feature_id=feature_id)
            else:
                data = FeatureRequest.to_collection_dict(
                    FeatureRequest.query.filter_by(
                        client_id=client_id), page, per_page, 'feature_request', client_id=client_id)
            return jsonify(data)
