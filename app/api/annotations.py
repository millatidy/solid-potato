...
def feature_request_checker(f):
    ...
    def _args_checker(*args, **kwargs):
        ...
        r_args = (int(feature_id is not None), int(client_id is not None))

        if r_args == (0, 0):
            return jsonify(
                {'message': 'feature_id and client_id cannot be empty'})

        if (r_args[0] != r_args[1]):
            return jsonify(
                {'message': 'both feature_id and client_id are required'})
